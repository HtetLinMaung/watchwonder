...
pub async fn get_chat_message_by_id(
    ...
    let user_id: i32 = parsed_values[0].parse().unwrap();
    match chat::get_chat_message_by_id(message_id, user_id, &client).await {
        Ok(chat_session) => HttpResponse::Ok().json(DataResponse {
            code: 200,
            message: String::from("Chat message fetched successfully."),
            data: Some(chat_session),
        }),
        Err(err) => {
            println!("{:?}", err);
            HttpResponse::NotFound().json(BaseResponse {
                code: 404,
                message: String::from("Chat message not found!"),
            })
        }
    }
}
...
pub async fn delete_chat_session(
    ...
    match chat::get_chat_session_by_id(chat_id, user_id, 0, &client).await {
        Ok(_) => match chat::delete_chat_session(chat_id, user_id, role, &client).await {
            Ok(()) => HttpResponse::Ok().json(BaseResponse {
                code: 204,
                message: String::from("Chat session deleted successfully"),
            }),
            Err(e) => {
                eprintln!("Chat session deleting error: {}", e);
                return HttpResponse::InternalServerError().json(BaseResponse {
                    code: 500,
                    message: String::from("Error deleting chat session!"),
                });
            }
        },
        Err(err) => {
